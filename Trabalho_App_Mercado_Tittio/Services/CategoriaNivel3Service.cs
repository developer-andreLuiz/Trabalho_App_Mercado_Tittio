...
namespace Trabalho_App_Mercado_Tittio.Services
{
    ...
    {
        public static CategoriaNivel3Service instancia = new CategoriaNivel3Service();
        ...
        public async Task<List<CategoriaNivel3ServiceModel>> GetAll()
        {
            string dados = URL;
            ...
            if (response.IsSuccessStatusCode)//codigo 200
            {
                ...
                return JsonConvert.DeserializeObject<List<CategoriaNivel3ServiceModel>>(content);
            }
            return new List<CategoriaNivel3ServiceModel>();
        }
        public async Task<CategoriaNivel3ServiceModel> Get(int id)
        {
            string dados = URL + "/" + id;
            HttpClient client = GetClient();
            var response = await client.GetAsync(dados);
            ...
            {
                string content = await response.Content.ReadAsStringAsync();
                return JsonConvert.DeserializeObject<CategoriaNivel3ServiceModel>(content);
            }
            return new CategoriaNivel3ServiceModel();
        }
        public async Task Create(CategoriaNivel3ServiceModel obj)
        {
            string dados = URL;
        ...
        public async Task Update(CategoriaNivel3ServiceModel obj)
        {
